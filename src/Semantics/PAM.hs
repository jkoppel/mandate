...
instance (Lang l, Matchable (payload l)) => Matchable (AMRhs payload l) where
  getVars (AMLetComputation c f r) = getVars c `Set.union` getVarsExtComp f `Set.union` getVars r
  getVars (AMRhs p) = getVars p

  match (Pattern (AMLetComputation c1 f1 r1)) (Matchee (AMLetComputation c2 f2 r2)) =
      match (Pattern c1) (Matchee c2) >> matchExtComp (Pattern f1) (Matchee f2) >> match (Pattern r1) (Matchee r2)
  match (Pattern (AMRhs p1)) (Matchee (AMRhs p2)) = match (Pattern p1) (Matchee p2)

  refreshVars (AMLetComputation c f p) = AMLetComputation <$> refreshVars c <*> refreshVarsExtComp f <*> refreshVars p
  refreshVars (AMRhs p) = AMRhs <$> refreshVars p

  fillMatch (AMLetComputation c f p) = AMLetComputation <$> fillMatch c <*> fillMatchExtComp f <*> fillMatch p
