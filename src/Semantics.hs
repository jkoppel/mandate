...
type family RedState l (v :: OpenClosed) :: *
data GConfiguration l s v = Conf { confTerm :: Term l v, confState :: s}
  deriving (Eq, Ord)

type Configuration l v = GConfiguration l (RedState l v) v

instance {-# OVERLAPPABLE #-} (Show s) => Show (GConfiguration l s v) where
  showsPrec d (Conf t s) = showString "(" . showsPrec d t . showString ", " . showsPrec d s . showString ")"

instance {-# OVERLAPPING #-} Show (GConfiguration l () v) where
